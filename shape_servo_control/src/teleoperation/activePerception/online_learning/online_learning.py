...
def grasp_state(sim_cache, robot_list, dc_client, learning_config, args):
    ...
    obj_particles = get_object_particle_state(gym, sim, data_config, sim_cache, vis=True)
...
def retract_state(sim_cache, data_config, robot_list, dc_client, xyz_list, learning_config, args):
    ...
    down_sampled_pcd = farthest_point_sample_batched(point=pcd, npoint=1024)
    down_sampled_pcd = torch.tensor(down_sampled_pcd).permute(0,2,1).float().to(device)
    query_points = learning_config["hinge_points"].to(device)
...
if __name__ == "__main__":
    ...
    x_ax = np.linspace(start=xy_lower_bound[0], stop=xy_upper_bound[0], num=(int)(np.sqrt(n_hinge_points)))
    y_ax = np.linspace(start=xy_lower_bound[1], stop=xy_upper_bound[1], num=(int)(np.sqrt(n_hinge_points)))
    x_grid, y_grid = np.meshgrid(x_ax, y_ax) #(n_query.sqrt,n_query.sqrt)
    grid = np.concatenate((x_grid[:,:,np.newaxis], y_grid[:,:,np.newaxis]), axis=-1) #(n_query.sqrt,n_query.sqrt,2)
    grid_points = grid.reshape((-1,2))
    

    ####### get ground truth observations of hinge points ######
    ...
    grid_expand = grid_points[:,np.newaxis, :] #(n_query, 1, 2)

    #print("++++++++++++++++++++++++++++++++++++++ooo", grid_expand.shape)

    dists = np.linalg.norm(balls_xy - grid_expand, axis=-1) #(n_query, num_attachment_points)
    ...
    #################################################
    hinge_points = torch.tensor(np.pad(grid_points, ((0,0), (0,1)))).float()
    ...
    learning_config = {"gamma": 0.5, "hinge_points":hinge_points, "hinge_points_labels":is_attachment_pt, "total_time":timesteps, "current_time": 0, "num_EM_iters":5, \
                        "sbhm": sbhm, "device": device}
