...
def record_state(sim_cache, data_config, args, record_pc=True):
    ...
    if record_pc:
        ...
        down_sampled_pcd = farthest_point_sample_batched(point=pcd, npoint=num_pts)
    ...
    full_data_path = os.path.join(args.rigid_state_path, "full_data")
    with open(os.path.join(full_data_path, f"group {args.group_count}.pickle"), 'rb') as handle:
    ...
    balls_relative_xyzs = np.array(data["balls_relative_xyz"])
    soft_xyz = np.array(data["soft_xyz"])
    balls_relative_xyzs[:,:2] = balls_relative_xyzs[:,:2] + soft_xyz[:2]
    balls_xyz = balls_relative_xyzs
