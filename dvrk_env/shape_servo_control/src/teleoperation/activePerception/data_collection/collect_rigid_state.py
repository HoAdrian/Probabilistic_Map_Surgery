...
from utils import plot_2D_points, get_2D_grid_points, compute_occluded_points #is_occluded, compute_camera_intrinsics_matrix, compute_camera_extrinsics_matrix
...
def configure_isaacgym(gym, args):
    ...
    # cam_targets = gymapi.Vec3(0.0, -0.4, 0.0)
    # cam_positions = gymapi.Vec3(0.0, -0.3, 0.0)
    ...
    if not args.headless:
        return envs, sim, dvrk_handles, dvrk_1_handles,  dvrk_2_handles, dvrk_1_pose, dvrk_2_pose, soft_handles, cam_handles, cam_prop, viewer, balls_relative_xyz, soft_pose, cam_targets, cam_positions
    else:
        return envs, sim, dvrk_handles, dvrk_1_handles,  dvrk_2_handles, dvrk_1_pose, dvrk_2_pose, soft_handles, cam_handles, cam_prop, None, balls_relative_xyz, soft_pose, cam_targets, cam_positions
...
def retract_state(sim_cache, data_config, robot_list, dc_client, xyz_list, args):
    ...
    if args.save_data:
        ...
        obj_particles = get_object_particle_state(gym, sim, data_config, sim_cache, vis=False).astype(np.float64)
        ...
        cam_handle = sim_cache["cam_handles"][0]
        cam_prop = sim_cache["cam_prop"]
        cam_targets = sim_cache["cam_targets"]
        cam_positions = sim_cache["cam_positions"]
        cam_positions = np.array([cam_positions.x, cam_positions.y, cam_positions.z])
        soft_xyz = np.array([soft_pose.p.x, soft_pose.p.y, soft_pose.p.z])
        xy_lower_bound = soft_xyz[:2] - 0.1
        xy_upper_bound = soft_xyz[:2] + 0.1
        grid = get_2D_grid_points(1024, xy_lower_bound, xy_upper_bound)
        grid_3D = np.pad(grid, ((0,0), (0,1))).astype(np.float64) #(1024,3)

        pcd = get_partial_pointcloud_vectorized(gym, sim, envs[0], cam_handles[0], cam_prop, color=[1,0,0], min_z = 0.005, visualization=True, min_depth=-1)

        occluded_idxs = compute_occluded_points(obj_particles, tri_indices, grid_3D, ray_origin=cam_positions, vis = True)

        labels = np.zeros((len(grid),))
        if len(occluded_idxs)!=0:
            labels[occluded_idxs] = 1
            plot_2D_points(grid_3D, labels, vmin=0, vmax=1, title="occlude", path=None, name=None, has_vmin=True, has_vmax=True, vis=True)


        # save other data
        data = {"vertices":obj_particles, "num_vertices": obj_particles.shape[0], "cam_pos":cam_positions, "tri_indices": tri_indices, "balls_relative_xyz": data_config["balls_relative_xyz"], "soft_xyz": [soft_pose.p.x, soft_pose.p.y, soft_pose.p.z]}
...
if __name__ == "__main__":
    ...
    envs, sim, dvrk_handles, dvrk_1_handles,  dvrk_2_handles, dvrk_1_pose, dvrk_2_pose, soft_handles, cam_handles, cam_prop, viewer, balls_relative_xyz, soft_pose, cam_targets, cam_positions = configure_isaacgym(gym, args)
    ...
    sim_cache = sim_cache = {"gym":gym, "sim":sim, "envs":envs, "dvrk_1_handles":dvrk_1_handles, "dvrk_2_handles":dvrk_2_handles, \
                ...
                "dvrk_1_pose": dvrk_1_pose, "dvrk_2_pose": dvrk_2_pose, "soft_pose": soft_pose, "cam_targets": cam_targets, "cam_positions": cam_positions}
