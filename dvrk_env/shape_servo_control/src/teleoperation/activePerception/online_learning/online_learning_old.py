...
def retract_state(sim_cache, data_config, robot_list, dc_client, xyz_list, learning_config, args):
    ...
    partial_pc = get_partial_pointcloud_vectorized(gym, sim, envs[0], cam_handles[0], cam_prop, color=[1,0,0], min_z = 0.005, min_depth=-1, visualization=True)
    ...
    # X = query_points[:,:2].type(torch.DoubleTensor)
    # Phi = rbf_kernel(X, hinge_points, gamma=learning_config["gamma"], bias_trick=learning_config["bias_trick"])
    # sbhm_target = torch.tensor(query_points_labels).reshape(-1,1).type(torch.DoubleTensor)
    # sbhm.EM_algo(sbhm_target, Phi, num_iters=learning_config["num_EM_iters"])
    # W = sbhm.sample_weights()

    X = torch.tensor(query_points[:,:2].type(torch.DoubleTensor))
    Phi = sbhm.get_rbf_features(X)
    sbhm_target = torch.tensor(pred_idx).reshape(-1,1).type(torch.DoubleTensor)
    sbhm.EM_algo(sbhm_target, Phi, num_iters=learning_config["num_EM_iters"])
    W = sbhm.sample_weights()

    # print(f"posterior covariance: {sbhm.covariance}")
    # print(f"posterior mean: {sbhm.mean}")

    _, sbhm_pred_mean, sbhm_pred_std = sbhm.predict(Phi, W)
    ...
    plot_2D_points(query_points, sbhm_pred_mean, vmin=0, vmax=1, title=f"sbhm mean prediction (at hinge points) at time {current_time}", path= f"{args.data_recording_path}/{args.object_name}/{current_time}", name="sbhm_prediction")
    plot_2D_points(query_points, sbhm_pred_std, vmin=0, vmax=math.inf, title=f"sbhm prediction std (at hinge points) at time {current_time}", path= f"{args.data_recording_path}/{args.object_name}/{current_time}", name="sbhm_std")
...
if __name__ == "__main__":
    ...
    query_points_labels = get_point_label_nn(balls_xy, query_points_2D, num_positive_points=50) #(n_query_points,)
    ...
    n_hinge_points = 64
    hinge_points = torch.tensor(get_2D_grid_points(n_hinge_points, xy_lower_bound, xy_upper_bound)).type(torch.DoubleTensor)
    ###############################################################################################################

    M = hinge_points.shape[0]
    timesteps = 2
    
    variances = 1000*torch.ones((M,)).type(torch.DoubleTensor)
    S_0 = torch.diag(variances).type(torch.DoubleTensor)
    m_0 = torch.zeros((M,1)).type(torch.DoubleTensor)
    xi = torch.ones((n_query_points,1)).type(torch.DoubleTensor)
    ...
    learning_config = {"num_EM_iters":10, "query_points":query_points, "query_points_labels":query_points_labels, \
