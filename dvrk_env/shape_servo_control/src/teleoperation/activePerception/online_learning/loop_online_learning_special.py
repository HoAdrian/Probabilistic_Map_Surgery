...
if __name__=="__main__":
    ...
    num_tissue = 1 #len(os.listdir(args.object_urdf_path))//2
    ...
    for i in range(num_tissue):
        print(f"== $$$$$$$$$$$ online learning for tissue_{i} $$$$$$$$$$$$ ==")
        os.system(f"python3 online_learning_special.py --data_recording_path={args.data_recording_path} --object_urdf_path={args.object_urdf_path} --object_mesh_path={args.object_mesh_path} --headless={args.headless} --save_data={args.save_data} --rand_seed={args.rand_seed} --object_name=tissue_{i} --weight_path={args.weight_path}")
        #i = len(os.listdir(os.path.join(args.data_recording_path, "full_data")))
