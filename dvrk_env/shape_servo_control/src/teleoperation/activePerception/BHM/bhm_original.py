...
class SBHM(LinearClassifierMixin, BaseEstimator):
    ...
    def sampling_predict(self, X_q, num_samples):
        Phi = torch.tensor(self.__sparse_features(X_q))
        normal = MultivariateNormal(torch.tensor(self.mu).reshape(-1), torch.tensor(self.sigma_[0]))
