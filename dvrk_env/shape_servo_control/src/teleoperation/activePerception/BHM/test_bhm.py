...
if __name__ == "__main__":
    ...
    train_points_labels = get_point_label_nn(balls_xy, train_points_2D, num_positive_points=200) #(n_query_points, )
    ...
    sbhm = SBHM(S_0, m_0, xi, hinge_points, gamma, bias_trick)
    ...
    test_Phi = sbhm.get_rbf_features(qX)
    ...
    plot_2D_points(X, train_points_labels, vmin=0, vmax=1, title=f"ground truth debug", path= f"{plot_path}/debug", name="ground_truth_prediction_debug")
    plot_2D_points(qX, sbhm_pred_mean, vmin=0, vmax=1, title=f"sbhm mean prediction debug", path= f"{plot_path}/debug", name="sbhm_prediction_debug")
    plot_2D_points(qX, sbhm_pred_std, vmin=0, vmax=math.inf, title=f"sbhm prediction std debug", path= f"{plot_path}/debug", name="sbhm_std_debug", has_vmin=True, has_vmax=False)
    #plot_2D_points(qX, laplace_pred, vmin=0, vmax=1, title=f"sbhm laplace prediction debug", path= f"{plot_path}/debug", name="sbhm__laplace_prediction_debug")
