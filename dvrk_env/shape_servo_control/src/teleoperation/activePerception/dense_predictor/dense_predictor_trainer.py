...
def train(model, device, train_loader, optimizer, epoch):
    ...
    for batch_idx, sample in enumerate(train_loader):
        ...
        query = sample[1].to(device) #(B, 3, num_pts)
        target = sample[2].to(device) #(B, num_query_pts)
        
        
        optimizer.zero_grad()
        output = model(pc, query) #(B, num_classes, num_query_pts)
...
if __name__ == "__main__":
    ...
    train_dataset = DensePredictorDataset(dataset_path=tdp)
